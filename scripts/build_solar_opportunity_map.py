...
def main() -> int:
    ...
    for prospect_id in dict.fromkeys(args.prospect_id):
        try:
            materialized = materialize_with_resource_observation(
                prospect_id,
                build=build_solar_opportunity_map,
                write=write_solar_opportunity_map,
            )
            payload = materialized["payload"]
            paths = materialized["artifacts"]
            rows.append({
                ...
                "resource_observation_path": (
                    materialized["resource_observation_path"]
                ),
                "resource_observation": (
                    materialized["resource_observation"]
                ),
