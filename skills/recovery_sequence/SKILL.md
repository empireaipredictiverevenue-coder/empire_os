...
(T1 day0, T2 day3, T3 day7). Each touch emails the buyer their exact BSC
Pay link so they can fund the seat in USDT. Idempotent — each touch fires once.
...
## Steps
1. Load .env (BSC_VAULT_WALLET + DB path).
...
4. get_pay_url(tenant_id): reconstruct `bsc:<vault>?memo=<payment_ref>&amount=<usdc>`
...
- BSC_VAULT_WALLET must be loaded from .env (recovery script does this;
