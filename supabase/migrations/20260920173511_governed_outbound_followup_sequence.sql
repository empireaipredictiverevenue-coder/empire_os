-- Governed no-reply follow-up sequence.
-- Two follow-ups maximum: +72h and +168h from the first delivered touch.
-- Replies, suppressions, bounce/complaint/failure evidence stop the sequence.
BEGIN;
...
WITH roots AS (
    ...
    WHERE i.channel='email'
      ...
      AND COALESCE(i.metadata->>'sequence_kind','first_touch') <> 'followup'
    ...
    SELECT
        ...
        CASE
          WHEN e.followup_count=0
               AND e.delivered_at <= clock_timestamp()-interval '72 hours'
            THEN 1
          WHEN e.followup_count=1
               AND e.first_followup_delivered_at IS NOT NULL
               AND e.delivered_at <= clock_timestamp()-interval '168 hours'
            THEN 2
          ELSE NULL
...
SELECT COALESCE(
    jsonb_agg(
        jsonb_build_object(
            'root_intent_id',d.root_intent_id,
            'review_id',d.metadata->>'buyer_candidate_review_id',
            'recipient',d.recipient,
            'normalized_recipient',d.normalized_recipient,
            'offer_key',d.offer_key,
            'root_subject',d.root_subject,
            'delivered_at',d.delivered_at,
            'followup_step',d.followup_step,
            'candidate_evidence',d.metadata->'candidate_evidence',
            'company_score',d.metadata->'company_score',
            'decision_score',d.metadata->'decision_score'
        )
        ORDER BY d.delivered_at,d.root_intent_id
    ),
    '[]'::jsonb
)
FROM (
    SELECT *
    FROM due
    WHERE followup_step IS NOT NULL
...
BEGIN
    ...
    IF p_step=1 AND delivered_at > clock_timestamp()-interval '72 hours' THEN
    ...
    IF p_step=2 THEN
        ...
        END IF;
        IF delivered_at > clock_timestamp()-interval '168 hours' THEN
            RAISE EXCEPTION 'final follow-up cooldown active';
        END IF;
    END IF;
...
BEGIN
    ...
    IF step=1 AND delivered_at > clock_timestamp()-interval '72 hours' THEN
        RAISE EXCEPTION 'first follow-up cooldown active';
    END IF;
    ...
    IF step=2 THEN
        ...
        IF first_followup_delivered IS NULL
           OR delivered_at > clock_timestamp()-interval '168 hours' THEN
...
REVOKE ALL ON FUNCTION public.propose_outbound_followup(
    uuid,integer,text,text,text,text,timestamptz
) FROM PUBLIC,anon,authenticated,empire_outbound_approver,
       empire_outbound_sender,empire_reply_ingest;
REVOKE ALL ON FUNCTION public.auto_approve_outbound_followup(uuid,integer)
  FROM PUBLIC,anon,authenticated,empire_outbound_approver,
       empire_outbound_sender,empire_reply_ingest;

GRANT EXECUTE ON FUNCTION public.list_due_outbound_followups(integer)
  TO service_role;
GRANT EXECUTE ON FUNCTION public.propose_outbound_followup(
    uuid,integer,text,text,text,text,timestamptz
) TO service_role;
GRANT EXECUTE ON FUNCTION public.auto_approve_outbound_followup(uuid,integer)
  TO service_role;

COMMENT ON FUNCTION public.list_due_outbound_followups(integer) IS
'Lists only delivered first-touch emails due for bounded no-reply follow-up.';
COMMENT ON FUNCTION public.propose_outbound_followup(
    uuid,integer,text,text,text,text,timestamptz
) IS
'Creates one of at most two no-reply follow-ups after cooldown and evidence checks.';
COMMENT ON FUNCTION public.auto_approve_outbound_followup(uuid,integer) IS
'Approves only due no-reply follow-up intents after suppression, reply and negative-delivery checks.';
