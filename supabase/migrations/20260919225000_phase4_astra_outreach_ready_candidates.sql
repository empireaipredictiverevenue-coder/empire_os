-- Phase 4 operational evidence hardening.
-- Only explicitly outreach-ready pending buyer candidates drive Astra's
-- buyer-outreach recommendation; all pending reviews remain auditable.
BEGIN;
...
failed AS (
  ...
  WHERE j.status='failed'
    AND coalesce(j.created_by,'') NOT IN (
      'manual_production_diagnostic',
      'manual_production_smoke'
    )
),
diagnostic_failed AS (
  SELECT count(*)::bigint AS value
  FROM public.gtm_jobs j
  WHERE j.status='failed'
    AND coalesce(j.created_by,'') IN (
      'manual_production_diagnostic',
      'manual_production_smoke'
    )
),
buyer_due AS (
  ...
  WHERE r.status='pending'
    AND coalesce((r.evidence->>'outreach_ready')::boolean,false)=true
),
buyer_pending_total AS (
  SELECT count(*)::bigint AS value
  FROM public.buyer_candidate_reviews r
  WHERE r.status='pending'
)
SELECT jsonb_build_object(
  ...
  'diagnostic_failed_jobs', diagnostic_failed.value,
  ...
  'buyer_reviews_pending_total', buyer_pending_total.value,
  'observed_at', clock_timestamp()
)
FROM owned,qualified,capacity,reply_backlog,failed,diagnostic_failed,buyer_due,buyer_pending_total;
$$;
...
'Phase 4 read-only canonical operational evidence for Astra; known diagnostic failures and non-outreach-ready pending buyer reviews are audit-visible but do not drive live action priority.';
