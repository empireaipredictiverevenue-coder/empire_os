...
-- Known smoke/diagnostic GTM failures remain auditable but no longer drive
-- the live failed-jobs priority signal.
BEGIN;
...
FROM owned,qualified,capacity,reply_backlog,failed,diagnostic_failed,buyer_due;
$$;
...
'Phase 4 read-only canonical operational evidence for Astra; known smoke/diagnostic GTM failures are exposed separately and do not drive live failed-job priority.';
