-- Fix Phase 4 buyer-scout raw prospect promotion on PostgreSQL builds
-- without min(uuid). Preserve duplicate-safe deterministic lookup.
-- Applied to canonical Supabase as fix_buyer_scout_uuid_promotion_lookup.

CREATE OR REPLACE FUNCTION public.promote_buyer_scout_candidate_to_prospect(
  p_candidate_id uuid,
  p_actor text
) RETURNS jsonb
LANGUAGE plpgsql
...
BEGIN
  ...
  IF NOT FOUND THEN
    RETURN jsonb_build_object(
      'decision','candidate_not_found','candidate_id',p_candidate_id,
      'database_write_performed',false,'canonical_promotion_performed',false,
      'qualification_created',false,'buyer_created',false,
      'outbound_sent',false,'actual_revenue',false
    );
  END IF;
  ...
  IF candidate.review_state <> 'review_ready'
    ...
    RETURN jsonb_build_object(
      'decision','blocked_not_review_ready','candidate_id',candidate.id,
  ...
  IF candidate.canonical_buyer_id IS NOT NULL THEN
    RETURN jsonb_build_object(
      ...
      'candidate_id',candidate.id,'canonical_buyer_id',candidate.canonical_buyer_id,
      'database_write_performed',false,'canonical_promotion_performed',false,
      'qualification_created',false,'buyer_created',false,
      'outbound_sent',false,'actual_revenue',false
    );
  ...
  IF candidate.canonical_prospect_id IS NOT NULL THEN
    IF EXISTS (
      ...
      RETURN jsonb_build_object(
        'decision','already_linked_promoted','candidate_id',candidate.id,
        'canonical_prospect_id',candidate.canonical_prospect_id,
        'prospect_created',false,'database_write_performed',true,
  ...
  IF normalized_name='' OR normalized_domain=''
    ...
    RETURN jsonb_build_object(
      'decision','blocked_identity_incomplete','candidate_id',candidate.id,
      'database_write_performed',false,'canonical_promotion_performed',false,
      'qualification_created',false,'buyer_created',false,
      'outbound_sent',false,'actual_revenue',false
    );
  ...
  IF normalized_name IN (
      ...
      'our services','welcome','homepage','index','learn more','read more',
      'click here','get started','step 1','step 2','step 3'
     )
     OR normalized_name ~
  ...
  THEN
    RETURN jsonb_build_object(
      ...
      'candidate_id',candidate.id,'business_name',candidate.business_name,
      'database_write_performed',false,'canonical_promotion_performed',false,
      'qualification_created',false,'buyer_created',false,
      'outbound_sent',false,'actual_revenue',false
    );
  ...
  WHERE regexp_replace(
          lower(split_part(split_part(
            regexp_replace(trim(COALESCE(p.website,'')),'^https?://','','i'),
            '/',1),':',1)),
          '^www\.','','i'
        ) = normalized_domain;

  IF prospect_matches = 1 THEN
    SELECT p.id INTO prospect_id
    FROM public.prospects p
    WHERE regexp_replace(
            lower(split_part(split_part(
              regexp_replace(trim(COALESCE(p.website,'')),'^https?://','','i'),
  ...
  IF prospect_matches > 1 THEN
    RETURN jsonb_build_object(
      ...
      'candidate_id',candidate.id,'domain',normalized_domain,
  ...
  IF prospect_matches = 0 THEN
    ...
    INSERT INTO public.prospects(
      business_name,niche,metro,phone,website,buy_signal_score,status,notes,
      contact_name,contact_title,contact_source,contacted_at,contacted_status
    ) VALUES (
      candidate.business_name,derived_niche,derived_metro,first_party_phone,
      candidate.website,NULL,'new',
      'buyer_scout_candidate:'||candidate.id::text||'; research_evidence_only',
      contact_name,contact_title,
      CASE WHEN contact_name IS NOT NULL THEN 'first_party_site' ELSE NULL END,
      NULL,'not_contacted'
    )
  ...
  SET canonical_prospect_id=prospect_id,
      ...
      provenance=candidate.provenance || jsonb_build_object(
        'canonical_prospect_promotion',
        jsonb_build_object(
          'actor',trim(p_actor),'prospect_id',prospect_id,
          'prospect_created',prospect_created,'promoted_at',clock_timestamp(),
          'buy_signal_score_policy','UNKNOWN_NULL',
          'qualification_created',false,'buyer_created',false,
          'outreach_authorized',false,'actual_revenue',false
        )
      )
  WHERE id=candidate.id;

  RETURN jsonb_build_object(
    'decision',CASE WHEN prospect_created
      THEN 'raw_prospect_created' ELSE 'existing_prospect_linked' END,
    'candidate_id',candidate.id,'canonical_prospect_id',prospect_id,
    'prospect_created',prospect_created,'database_write_performed',true,
    'canonical_promotion_performed',true,
    'buy_signal_score_policy','UNKNOWN_NULL',
    'qualification_created',false,'buyer_created',false,
    'outbound_sent',false,'terms_accepted',false,'actual_revenue',false
  );
END;
$$;

REVOKE ALL ON FUNCTION public.promote_buyer_scout_candidate_to_prospect(uuid,text)
FROM PUBLIC,anon,authenticated;

GRANT EXECUTE ON FUNCTION public.promote_buyer_scout_candidate_to_prospect(uuid,text)
TO service_role;
