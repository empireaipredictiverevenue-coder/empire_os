...
def create_revenue_os_router(
    ...
    @router.post("/feedback/model-review/preview")
    def model_review_feedback_preview(req: RevenueOsLearningRequest):
        try:
            normalized = (
                req.now_utc[:-1] + "+00:00"
                if req.now_utc.endswith("Z")
            ...
            feedback = build_revenue_os_model_review_feedback(readiness)
        except ValueError as exc:
            raise HTTPException(status_code=422, detail=str(exc)) from exc
