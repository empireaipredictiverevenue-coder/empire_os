...
def re_score_existing(limit: int = 500) -> int:
    ...
    with sqlite3.connect(DB) as c:
        ...
        rows = c.execute(
            "SELECT prospect_id, niche, metro, score "
            "FROM si_buyer_outreach "
            "WHERE score < 80 OR last_touch_at < datetime('now', '-1 day') "
        ...
        for pid, niche, metro, old_score in rows:
            ...
            if new_score != old_score:
                c.execute(
                    "UPDATE si_buyer_outreach SET score=, last_touch_at=datetime('now') WHERE prospect_id=?",
                    (new_score, pid)
                )
                updated += 1
        if updated:
