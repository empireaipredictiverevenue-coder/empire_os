...
# Auto-scale cap: never exceed Brevo daily quota (free = 300, leave headroom).
MAX_BATCH_PER_TICK = int(os.getenv("SALES_AGENT_MAX_BATCH", "100"))


def _effective_batch() -> int:
    """Cap BATCH at both MAX_BATCH_PER_TICK AND Brevo remaining quota."""
    import sys
    try:
        sys.path.insert(0, "/root/empire_os")
        from empire_os.brevo_quota import get_remaining
        remaining = get_remaining()
    except Exception:
        remaining = MAX_BATCH_PER_TICK
    return max(0, min(BATCH, MAX_BATCH_PER_TICK, remaining - 5))
...
def _pick_variant(prospect_id: str) -> str:
    """Deterministic A/B variant: hash prospect_id -> 'A' or 'B'.

    Split ~50/50. Same prospect always gets same variant (no thrash).
    """
    import hashlib
...
def _render_email(prospect: dict, quote: dict, variant: str) -> tuple:
    """Returns (subject, body) for the given variant."""
    product = quote.get("product", "lead_lane").replace("_", " ").title()
    amount = quote.get("amount_usdc", 0)
    pay_url = quote.get("pay_url", "")
    quote_id = quote.get("quote_id", "")
    expires = quote.get("expires_at", "in 30 min")
    biz_name = prospect.get("business_name", prospect.get("name", "there"))
    niche = (prospect.get("niche") or quote.get("product", "your business")).replace("_", " ").title()
    metro = prospect.get("metro", "")

    if variant == "B":
        subject = f"{niche} offer tailored for {biz_name}"
        body = (
            f"Hi,\n\n"
...
def send_quote_email(quote: dict, prospect: dict) -> dict:
    ...
    # A/B variant: deterministic hash so same prospect always sees same variant.
    variant = _pick_variant(prospect.get("prospect_id", email))
    subject, body = _render_email(prospect, quote, variant)
...
def run_once() -> dict:
    ...
    prospects = warm_prospects(eff_batch) if eff_batch > 0 else []
