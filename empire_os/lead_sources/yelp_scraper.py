"""Self-hosted Yelp scraper — no API key, Playwright-based."""
...
from sources.yelp_scraper import YelpScraper
...
def run(metro: str = None, verticals: list = None, limit: int = 100) -> Iterator[LeadCandidate]:
    """Run self-hosted Yelp scraper for given metro/niches."""
    ...
    for niche in niches:
        ...
        for query in queries:
            try:
                ...
                print(f"[yelp] Error for {query} in {state}: {e}")
...
def register_source(reg):
    ...
    reg(SourceInfo(
        ...
        tier="real",
        requires=[],
        description="Self-hosted Yelp scraper (Playwright) — no API key",
        run_fn=run,
    ))