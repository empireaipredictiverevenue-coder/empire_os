...
For each expired a2a_quote above a size threshold we rebuild a Solana payment
...
HUB_URL = os.getenv("EMPIRE_HUB_URL", "http://localhost:8081")
VAULT = os.getenv(
    "SOLANA_VAULT_WALLET",
...
def db() -> sqlite3.Connection:
    # The hub keeps an active WAL writer; tolerate lock contention.
...
def build_pay_url(quote_id: str, amount: float) -> str:
    """Solana pay URI — Vault + memo + amount in USDC."""
...
def build_body(quote_id: str, product: str, amount: float, pay_url: str) -> str:
    ...
    return (
        ...
        f"(${amount:,.2f} USDC) was set to expire, but we've kept the seat open "
        f"for you.\n\n"
        f"Pay here to activate it (single tap in any Solana wallet):\n"
        f"  {pay_url}\n\n"
                ...
                for attempt in range(5):
                    try:
                        c.execute(
                            ...
                            VALUES (?, ?, ?, ?, 'queued', ?)
                            """,
                            (r["quote_id"], email, r["amount_usdc"],
                             out_id, _now()),
                        )
                        c.commit()
                        marked = True
                ...
                if not marked:
                    ...
                    if len(summary["error_samples"]) < 5:
                        summary["error_samples"].append(
                            {"quote_id": r["quote_id"],
                             "resp": {"ok": False,
                                      "error": "could not mark si_quote_reengage "
                                               "after enqueue"}}
                        )
                log_jsonl({"ts": _now(), "quote_id": r["quote_id"],
                           "to_email": email, "amount_usdc": r["amount_usdc"],
...
def main() -> int:
    ...
    ap.add_argument("--min-amount", type=float, default=0.0,
                    help="Skip quotes below this USDC amount")
