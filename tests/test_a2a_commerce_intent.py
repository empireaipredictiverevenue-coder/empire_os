...
def api_client(repository=None, verifier=None, trusted=None):
    app = FastAPI()
    app.include_router(
...
def request_body(idempotency_key="intent-key-1"):
    return {
        "identity": {
            "agent_id": "agent-buyer-1",
            "key_id": "key-1",
            "nonce": "nonce-1",
            "issued_at": "2026-09-19T20:30:00+00:00",
            "signature": "sig-1",
        },
...
def test_authenticated_intent_is_pending_approval_only():
    repo = FakeRepository()
    client = api_client(repo, valid_verifier, {"key-1"})
    response = client.post(
        "/v1/a2a-commerce/intents",
        json=request_body(),
    )
