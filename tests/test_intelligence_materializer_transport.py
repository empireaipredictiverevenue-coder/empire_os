...
def _public_review_signal():
    return {
        "schema_version": "intelligence_signal_candidate.v1",
        "entity_id": ENTITY_ID,
        "signal_type": "competitor_public_review_presence",
        "signal_domain": "competitive_intelligence",
        "observed_at": "2026-09-22T13:15:00+00:00",
        "source_id": SOURCE_ID,
        "strength": 0.666667,
        "confidence": 0.85,
        "payload": {
            ...
            "profiles": [
                {
                    "platform": "bbb",
                    "profile_url": "https://bbb.org/acme",
                    "source_ref": "https://acme.example",
                },
                {
                    "platform": "yelp",
                    "profile_url": "https://yelp.com/biz/acme",
                    "source_ref": "https://acme.example/reviews",
                },
            ],
            "research_candidate": True,
...
class PublicReviewSignalCursor(FakeCursor):
    def __init__(self, *, duplicate=False):
        super().__init__()
        self.duplicate = duplicate
    ...
    def execute(self, sql, params=None):
        ...
        if text.startswith("SET LOCAL ROLE"):
            ...
            self._set(["id"], [("review-signal-id",)])
        else:
            raise AssertionError(f"unexpected SQL: {text}")


def _public_review_writer(*, duplicate=False):
    cursor = PublicReviewSignalCursor(duplicate=duplicate)
    connect = FakeConnect(cursor)
    writer = PostgresIntelligenceMaterializer(
        "postgresql://materializer@example/db",
        connect_factory=connect,
...
def test_public_review_signal_persists_observe_only_shape():
    from empire_os.intelligence_materializer_transport import (
        persist_competitor_public_review_signal,
    )

    writer, cursor = _public_review_writer()
    result = persist_competitor_public_review_signal(
        writer,
        _public_review_signal(),
    )
    ...
    assert "'competitor_public_review_presence'" in insert_sql
...
def test_public_review_signal_dedupes_by_profile_evidence():
    from empire_os.intelligence_materializer_transport import (
        persist_competitor_public_review_signal,
    )

    writer, cursor = _public_review_writer(duplicate=True)
    result = persist_competitor_public_review_signal(
        writer,
        _public_review_signal(),
    )
...
def test_public_review_signal_rejects_sentiment_inference():
    from empire_os.intelligence_materializer_transport import (
        persist_competitor_public_review_signal,
    )

    writer, _ = _public_review_writer()
    signal = _public_review_signal()
    signal["payload"]["review_sentiment_inferred"] = True

    with pytest.raises(
        IntelligenceMaterializerTransportError,
        match="sentiment",
    ):
        persist_competitor_public_review_signal(writer, signal)



def _search_presence_signal():
    return {
        "schema_version": "intelligence_signal_candidate.v1",
        "entity_id": ENTITY_ID,
        "signal_type": "competitor_search_presence",
        "signal_domain": "search_intelligence",
        "observed_at": "2026-09-22T13:30:00+00:00",
        "source_id": SOURCE_ID,
        "strength": 0.666667,
        "confidence": 0.85,
        "payload": {
            "company_name": "Acme Roofing",
            "company_domain": "acme.example",
            "query_presence_count": 2,
            "observation_count": 2,
            "best_position": 2,
            "reciprocal_position_weight": 0.7,
            "observed_search_presence_share": 0.4,
            "observations": [
                {
                    "query": "Denver roofing",
                    "engine": "bing_html",
                    "position": 2,
                    "domain": "acme.example",
                    "url": "https://acme.example/roofing",
                    "observed_at": "2026-09-22T13:30:00+00:00",
                },
                {
...
class SearchPresenceSignalCursor(FakeCursor):
    def __init__(self, *, duplicate=False):
        super().__init__()
        self.duplicate = duplicate
    ...
    def execute(self, sql, params=None):
        ...
        if text.startswith("SET LOCAL ROLE"):
            ...
            if self.duplicate:
                self._set(
                    ["payload"],
                    [(_search_presence_signal()["payload"],)],
                )
            else:
                self._set(["payload"], [])
        elif text.startswith(
            ...
            self._set(["id"], [("search-presence-signal-id",)])
        else:
            raise AssertionError(f"unexpected SQL: {text}")


def _search_presence_writer(*, duplicate=False):
    cursor = SearchPresenceSignalCursor(duplicate=duplicate)
    connect = FakeConnect(cursor)
    writer = PostgresIntelligenceMaterializer(
        "postgresql://materializer@example/db",
        connect_factory=connect,
...
def test_search_presence_signal_persists_observe_only_shape():
    from empire_os.intelligence_materializer_transport import (
        persist_competitor_search_presence_signal,
    )

    writer, cursor = _search_presence_writer()
    result = persist_competitor_search_presence_signal(
        writer,
        _search_presence_signal(),
    )
    ...
    assert "'competitor_search_presence'" in insert_sql
    assert "'search_intelligence'" in insert_sql
...
def test_search_presence_signal_dedupes_by_observations():
    from empire_os.intelligence_materializer_transport import (
        persist_competitor_search_presence_signal,
    )

    writer, cursor = _search_presence_writer(duplicate=True)
    result = persist_competitor_search_presence_signal(
        writer,
        _search_presence_signal(),
    )
...
def test_search_presence_signal_rejects_market_share_inference():
    from empire_os.intelligence_materializer_transport import (
        persist_competitor_search_presence_signal,
    )

    writer, _ = _search_presence_writer()
    signal = _search_presence_signal()
    signal["payload"]["market_share_inferred"] = True

    with pytest.raises(
        IntelligenceMaterializerTransportError,
        match="market share",
    ):
        persist_competitor_search_presence_signal(writer, signal)


def test_search_presence_signal_rejects_execution_authority():
    from empire_os.intelligence_materializer_transport import (
        persist_competitor_search_presence_signal,
    )
...
def _public_activity_signal():
    return {
        "schema_version": "intelligence_signal_candidate.v1",
        "entity_id": ENTITY_ID,
        "signal_type": "competitor_public_activity",
        "signal_domain": "competitive_intelligence",
        "observed_at": "2026-09-22T13:55:00+00:00",
        "source_id": SOURCE_ID,
        "strength": 0.5,
        "confidence": 0.85,
        "payload": {
            ...
            "company_domain": "acme.example",
            "observation_count": 2,
            "ads_offers_creative_count": 1,
            "hiring_count": 1,
            "event_count": 0,
            "company_activity_count": 0,
            "observations": [
                {
                    "category": "ads_offers_creative",
                    "evidence_key": "ads_offers_creative:https://acme.example/specials",
                    "source_ref": "https://acme.example/specials",
                },
                {
                    "category": "hiring",
                    "evidence_key": "hiring:https://acme.example/careers",
                    "source_ref": "https://acme.example/careers",
                },
            ],
            "research_candidate": True,
            "buyer_intent": False,
...
class PublicActivitySignalCursor(FakeCursor):
    def __init__(self, *, duplicate=False):
        super().__init__()
        self.duplicate = duplicate
    ...
    def execute(self, sql, params=None):
        ...
        if text.startswith("SET LOCAL ROLE"):
            ...
            if self.duplicate:
                self._set(
                    ["payload"],
                    [(_public_activity_signal()["payload"],)],
                )
            else:
                self._set(["payload"], [])
        elif text.startswith(
            ...
            self._set(["id"], [("public-activity-signal-id",)])
        else:
            raise AssertionError(f"unexpected SQL: {text}")


def _public_activity_writer(*, duplicate=False):
    cursor = PublicActivitySignalCursor(duplicate=duplicate)
    connect = FakeConnect(cursor)
    writer = PostgresIntelligenceMaterializer(
        "postgresql://materializer@example/db",
        connect_factory=connect,
...
def test_public_activity_signal_persists_observe_only_shape():
    from empire_os.intelligence_materializer_transport import (
        persist_competitor_public_activity_signal,
    )

    writer, cursor = _public_activity_writer()
    result = persist_competitor_public_activity_signal(
        writer,
        _public_activity_signal(),
    )
    ...
    assert "'competitor_public_activity'" in insert_sql
...
def test_public_activity_signal_dedupes_by_observations():
    from empire_os.intelligence_materializer_transport import (
        persist_competitor_public_activity_signal,
    )

    writer, cursor = _public_activity_writer(duplicate=True)
    result = persist_competitor_public_activity_signal(
        writer,
        _public_activity_signal(),
    )
...
def test_public_activity_signal_rejects_execution_authority():
    from empire_os.intelligence_materializer_transport import (
        persist_competitor_public_activity_signal,
    )

    writer, _ = _public_activity_writer()
    signal = _public_activity_signal()
    signal["execution_authority"] = "outbound"
    ...
    with pytest.raises(
        ...
        persist_competitor_public_activity_signal(writer, signal)
