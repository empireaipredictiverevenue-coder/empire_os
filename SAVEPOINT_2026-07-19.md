...
- Real money = USDT crypto payouts (BSC). PayPal email offcicialphilliplivesley@gmail.com is identifier ONLY — never paste private keys, public wallet addr only.
...
| recovery_sequence.py (3-touch USDT recovery on $298k uncollected) | /root/empire_os/empire_os/agents/recovery_sequence.py | LIVE, daily 09:00 timer |
...
| payout engine (BSC USDT transfer) | /root/empire_os/empire_os/payout.py + payout_batch.py | CODE READY, NOT TRIGGERED (no hourly scheduler yet) |
...
2. **Hourly payout scheduler:** build `payout_scheduler.py` (systemd timer, hourly) → pays settled invoices to wallet via `payout.usdc_transfer`. Needs BSC_PAYER_SECRET + funded vault (NOT yet set).
...
5. **First real USDT payment** closes the loop (S4/S5 currently 0).
...
- BSC_VAULT_WALLET: container .env (public addr). PAYER_SECRET: NOT SET — required for payouts.
