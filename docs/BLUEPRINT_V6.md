...
- Buyer-discovery website evidence now shares the first-party rule with Lead Scoring/Search Fabric: directory/social platform URLs contribute no website score, cannot seed generated work-email patterns, and are never probed as company sites. The OBSERVE preview also supports canonical `--niche` / `--metro` market scoping before ranking or probing. ✅ local/tested
