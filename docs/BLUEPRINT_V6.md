...
- Lead Scoring v2 schema is active in canonical Supabase. One explicitly approved bounded All Star Roofing v2 qualification now coexists with its unchanged v1 row; bulk v2 writes and any buyer-allocation cutover remain gated.
...
- Remaining production gates: Intelligence Materializer migration/runtime credential activation, bulk v2 qualification, security-lockdown migrations, buyer-allocation cutover, outbound contact, commercial terms and payment execution.
