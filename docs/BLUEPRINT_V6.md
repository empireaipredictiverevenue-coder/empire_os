...
- Evidence-backed singleton identity planning is local/tested and fail-closed; it does not relax the bulk duplicate resolver. All Star Roofing (Austin) is the first Phase 3F dry-run candidate to satisfy accepted direct acquisition provenance + exact phone/name + verified first-party domain corroboration, and its evidence-aware replay reaches quality 86.3 with decision confidence 0.55. Canonical entity/link promotion, v2 persistence, materialization and outreach remain production-gated. ✅ local/tested
