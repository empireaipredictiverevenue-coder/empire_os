...
- Lead Scoring v2 schema is active in canonical Supabase. One explicitly approved bounded All Star Roofing v2 qualification now coexists with its unchanged v1 row. Buyer allocation now prefers v2 with v1 fallback and independently enforces the v2 evidence-confidence floor; allocation execution and bulk v2 writes remain gated. ✅ local/tested consumer cutover
