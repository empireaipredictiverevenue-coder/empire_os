...
- Lead Scoring v2 evidence-aware qualification is local/tested: missing evidence remains unknown instead of zero, quality and evidence confidence are separated, and low-confidence records become insufficient_evidence. Read-only production replay of the 466 identity+qualification prospects raised average underlying quality from 37.06 to 61.90 but left all 466 below the 0.50 decision-confidence floor (average 0.3028), proving targeted enrichment is required before governed commercial selection. ✅ local/tested
- Lead Scoring v2 schema activation, v2 writes, and any buyer-allocation cutover remain gated; v1 stays intact in parallel.
...
- Bounded real-web enrichment execution remains separate from planning; evidence must pass Search Fabric discovery + identity guard + site-probe checks before v2 can count it.
