# EmpireOS Master Remaining Checklist

_Last saved: 2026-09-22_

This is the canonical execution checklist for the remaining EmpireOS build.

**DONE rule:** code + tests + live runtime + canonical data + Founder surface where applicable. A file existing by itself is not completion.

## Current checkpoint

Competitor-audience intelligence is live through Account Intelligence Brief + Claim Critic. Buyer State, Next-Best Action and the Account / Buyer Digital Twin are live and verified in the Founder Console.

Current truth:
- research evidence is not buyer intent
- buyer/commercial intent remain false unless observed
- forecasts/scores are not revenue
- payment request is not payment
- payment is not recognized revenue
- recognized revenue is not realized gross profit

## Competitor Audience Graph

- [x] Public evidence discovery
- [x] Company/entity resolution
- [x] Same-page competitor evidence
- [x] Canonical Supabase persistence
- [x] Dedupe across repeated sweeps
- [x] Relevance filtering / junk-result rejection
- [x] First-party direct-site observation
- [x] Founder Console projection

## Competitor Research Intelligence

- [x] Research-priority score
- [x] Evidence stacking
- [x] Omega/Cortex context adapter
- [x] Deep-account-research routing
- [x] Additional-evidence routing
- [x] Bounded public research executor
- [x] Account Intelligence Brief
- [x] Claim Critic
- [x] Unsupported commercial claims fail closed

## Buyer State Evidence Stack — DONE

- [x] Merge entity + prospect + qualification + supported claims
- [x] Represent factual states separately:
  DISCOVERED -> ICP_MATCH -> SIGNAL_ACTIVE -> RESEARCHED -> READY -> CONTACTED -> ENGAGED -> CONVERSATION -> COMMERCIAL_INTENT -> TERMS -> PAYMENT_PENDING -> PAID -> FULFILLED -> EXPANSION
- [x] Unknown states stay unknown
- [x] Competitor evidence must not equal buyer intent
- [x] Qualification score must not equal commercial intent
- [x] Expose buyer-state stack in Founder Console
- [x] Add tests against false promotion

## Next-Best Action engine — DONE

- [x] Research more
- [x] Enrich identity
- [x] Verify decision-maker
- [x] Prepare buyer review
- [x] Hold
- [x] Conversation follow-up
- [x] Terms candidate
- [x] Payment review
- [x] Fulfilment review
- [x] No autonomous jump across founder/commercial gates
- [x] Show NBA reasoning/evidence in Founder Console

## Account / Buyer Digital Twin — DONE

- [x] Company identity
- [x] Public evidence
- [x] Competitor relationships
- [x] Qualification history
- [x] Research history
- [x] Outreach history
- [x] Replies/conversation
- [x] Commercial state
- [x] Terms/payment/fulfilment
- [x] Outcomes
- [x] Realized revenue / GP
- [x] Uncertainty explicitly represented

## Cortex learning loop — IMPLEMENTATION DONE / REAL OUTCOME PROOF PENDING

- [x] Research evidence as features only
- [x] Replies as observed outcomes
- [x] Verified commercial outcomes as labels
- [x] Payment + fulfilment + recognized revenue required for verified-customer learning
- [x] No learning from forecasts as if they were outcomes
- [x] No synthetic commercial labels
- [x] Calibration / feedback loop

## Competitor mining at scale

- [x] More competitors per market
- [x] More Denver roofing companies
- [x] Customer/case-study ecosystem
- [x] Testimonial companies
- [x] Partner ecosystem
- [x] Public review/company overlap
- [x] Comparison pages
- [ ] Search overlap — PARKED: provider evidence unavailable; canonical roster is non-blocking
- [x] Ads/offers/creative tracking
- [x] Jobs/hiring signals
- [x] Events/webinars
- [x] Public company activity
- [x] Competitor audience overlap map
- [x] Underserved audience finder
- [x] Territory heatmap
- [ ] Share-of-voice / search presence — PARKED: optional SERP overlay; provider reliability does not block downstream intelligence
- [x] Feed TAM + Revenue GPS + GTM + Search Intelligence

## Automate recurring intelligence

- [ ] Scheduled competitor sweeps
- [ ] Evidence refresh
- [ ] Freshness/expiry
- [ ] Changed-page detection
- [ ] Research-on-change
- [ ] Automatic Founder snapshot refresh
- [ ] Service/timer health
- [ ] Bounded retry/provider fallback
- [ ] Search Fabric provider reliability improvement

## Buyer acquisition / commercial loop

- [ ] Continue real outbound
- [ ] Improve target quality using account intelligence
- [ ] Legitimate decision-maker enrichment
- [ ] Buyer review creation
- [ ] Governed outbound intent
- [ ] Genuine positive/commercial reply
- [ ] Evidence-backed one-page opportunity brief
- [ ] Capacity/territory/delivery-route capture
- [ ] Closer case
- [ ] Actual product-bound order
- [ ] Commercial terms
- [ ] Founder gate where required
- [ ] USDT/BSC payment request
- [ ] Verified payment evidence
- [ ] Fulfilment
- [ ] Outcome capture
- [ ] Revenue recognition
- [ ] Realized gross profit

## Managed Service first-cash offer

- [x] Offer defined
- [x] $1,500 pilot economics defined
- [x] Acquisition/fulfilment ceilings defined
- [ ] Move managed_service v2 through legitimate catalog verifier
- [ ] Verify commercial evidence rows
- [ ] Activate approved product version
- [ ] Bind first real buyer
- [ ] First verified paid pilot
- [ ] First fulfilled pilot
- [ ] First recognized revenue
- [ ] First measured GP

## Founder Console

- [x] Property surface
- [x] Private Capital surface
- [x] Permit surface
- [x] Competitor audience surface
- [x] Research-priority surface
- [x] Omega/Cortex context
- [x] Account research
- [x] Account Intelligence Brief
- [x] Buyer State
- [x] Next-Best Action
- [x] Account digital twin
- [x] Market opportunities
- [ ] Competitor graph visualization
- [ ] Real funnel
- [ ] Terms/payment/fulfilment
- [ ] Recognized revenue/GP
- [ ] Revenue Pulse
- [ ] Storm Leads Multiplier
- [ ] Market Sweeps
- [ ] Live system/service health
- [ ] Premium frontend/3D interaction work

## Revenue Pulse

- [ ] Canonical Supabase inputs only
- [ ] Real funnel conversion
- [ ] Real revenue
- [ ] Realized GP
- [ ] Leak detection
- [ ] Opportunity alerts
- [ ] Market/niche/territory rollups
- [ ] Founder dashboard integration

## Market Sweeps / Revenue GPS

- [ ] Scheduled market scanning
- [ ] Demand signals
- [ ] Competitor pressure
- [ ] Supply gaps
- [ ] Opportunity evidence
- [ ] Product recommendation
- [ ] Economics estimate clearly marked prediction
- [ ] Feed research/NBA
- [ ] Founder market-opportunity view

## Storm Leads Multiplier

- [ ] Restore/confirm storm tracker pipeline
- [ ] Storm/event detection
- [ ] Affected businesses/properties
- [ ] Scoring
- [ ] Enrichment
- [ ] Buyer/product matching
- [ ] Founder dashboard
- [ ] Commercial path
- [ ] No fake storm opportunities

## Permit Intelligence

- [x] Canonical evidence exists
- [ ] Convert observed permit signals into proper opportunities
- [ ] Buyer/product matching
- [ ] Economics
- [ ] Recurring feed/API product
- [ ] Monetization proof

## Property Intelligence

- [x] Evidence surface live
- [ ] Opportunity extraction
- [ ] Prioritization
- [ ] Economics
- [ ] Commercial offer
- [ ] Paid proof

## Private Capital / Roll-Up Intelligence

- [x] Runtime/snapshot infrastructure exists
- [ ] Acquire genuine canonical PE/company evidence
- [ ] Segment roll-up markets
- [ ] Target/member graph
- [ ] Opportunity detection
- [ ] Deal-intent evidence
- [ ] Enterprise offer
- [ ] Paid proof
- [ ] Never fabricate PE evidence

## Search / SEO / AEO / GEO product family

- [x] Core Search Intelligence foundation exists
- [ ] Finish native Rank-Math-style product
- [ ] Technical SEO audits
- [ ] Content opportunities
- [ ] SERP tracking
- [ ] AI citation tracking
- [ ] Competitor content gap
- [ ] AEO/GEO
- [ ] Reporting/dashboard
- [ ] Sellable plans/API/white-label
- [ ] Real customer proof

## TAM / Market Intelligence

- [ ] TAM graph from canonical companies
- [ ] ICP clustering
- [ ] Decision-maker enrichment
- [ ] Competitor-derived lookalikes
- [ ] Territory segmentation
- [ ] Niche/metro opportunities
- [ ] White-label feed/API
- [ ] TAM numbers backed by evidence

## Commercial/product economics across Empire

- [ ] Permit pricing
- [ ] Property pricing
- [ ] Search Intelligence pricing
- [ ] Storm pricing
- [ ] Data/API pricing
- [ ] Seats/territories
- [ ] Managed-growth pricing
- [ ] Enterprise/private-capital pricing
- [ ] White-label/channel pricing
- [ ] Usage/value-based pricing
- [ ] Actual cost tracking
- [ ] Realized margin tracking

## Production cleanup / infrastructure

- [ ] Inspect stray FETCH_HEAD file before removing it
- [ ] Continue leaving /recovery untouched
- [ ] Continue leaving /toop untouched
- [ ] Keep explicit-path Git staging only
- [ ] Service health/alerts
- [ ] Safe deployment path
- [ ] Direct Empire MCP/ops connection to reduce copy/paste
- [ ] No destructive authority expansion

## Phase exit / real-money proof

- [ ] Genuine buyer
- [ ] Genuine positive commercial conversation
- [ ] Accepted terms
- [ ] Verified USDT/BSC payment
- [ ] Fulfilled product/service
- [ ] Observed outcome
- [ ] Recognized revenue
- [ ] Realized GP
- [ ] Feed outcome back into Cortex
- [ ] Repeat with second customer
- [ ] Prove repeatability before scaling spend

## Execution order

1. Buyer State Evidence Stack
2. Next-Best Action
3. Account / Buyer Digital Twin
4. Cortex outcome loop
5. Scale competitor mining
6. Commercial conversion
7. Payment
8. Fulfilment
9. Recognized revenue / realized GP
